...
interface PlacesResultsTableProps {
...
export function PlacesResultsTable({
  places,
  selectedIds,
  onSelectionChange,
}: PlacesResultsTableProps) {
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>
                <button type="button" onClick={() => toggleSort("name")}>Business Name</button>
              </TableHead>
              <TableHead>Category</TableHead>
              <TableHead>
                <button type="button" onClick={() => toggleSort("city")}>City</button>
              </TableHead>
              <TableHead>
                <button type="button" onClick={() => toggleSort("rating")}>Rating</button>
              </TableHead>
            ...
            {pageItems.length === 0 ? (
              ...
              pageItems.map((place) => (
                <TableRow key={place.id}>
                  ...
                  <TableCell>{place.reviewCount ?? "—"}</TableCell>
                  <TableCell>{place.phone ?? "—"}</TableCell>
                  <TableCell className="max-w-[120px] truncate">{place.website ?? "—"}</TableCell>
                  <TableCell>{place.status}</TableCell>
                  <TableCell>
                    <Button variant="ghost" size="sm" nativeButton={false} render={<Link href={`/google-places/${place.id}`} />}>
                      View
                    </Button>
                  </TableCell>
                </TableRow>
